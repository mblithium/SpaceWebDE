(
    function desktop() {
        const desktopExists= document.querySelector("#desktop");
        if(desktopExists) { return };

        document.querySelector("#system").innerHTML = "";
        swdehandlesrc("loadstyle", "./lib/components/desktop/desktop.css", "desktop");

        const DESKTOP = swdeutils.CreateHTML(`
        ...
        note.innerHTML = `
            <p>V.${SW_SystemConfig.version}</p>
            <p>In Alpha, bugs may occur.</p>
